...
namespace HotelBookingOnline\API;
...
class OptionsAPI {
	 ...
	 * @var $namespace
	 */
	private $namespace = EDMS_BOOKING_SLUG;
	...
	public function get_option( WP_REST_Request $request ) {
		...
		if ( ! empty( $key ) ) {
			if ( $options->has( $key ) ) {
				...
				return new WP_Error( 'option_error', __( 'Invalid or expired option name.', 'edms_booking' ) );
			}
		} else {
			$result = $options->get();
		}
	...
	public function set_option( WP_REST_Request $request ) {
		...
		if ( ! empty( $settings ) && is_array( $settings ) ) {
			...
			return new WP_Error( 'settings_error', __( 'No settings provided.', 'edms_booking' ) );
		}

		return new WP_REST_Response(
			array(
				'message' => __( 'Settings updated.', 'edms_booking' ),
			),
			200
		);
	}
	...
	public function delete_option( WP_REST_Request $request ) {
		...
		if ( ! empty( $key ) ) {
			...
			return new WP_Error( 'option_error', __( 'No option key is provided.', 'edms_booking' ) );
		}

		return new WP_REST_Response(
			array(
				'message' => __( 'Setting deleted.', 'edms_booking' ),
			),
			200
		);
	}
